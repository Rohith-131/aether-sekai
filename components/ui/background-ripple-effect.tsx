...
export const BackgroundRippleEffect = ({
  ...
  return (
    <div
      ...
      className={cn("absolute inset-0 z-0 w-full h-full overflow-hidden", className)}
    >
      {/* RED & PINK BLEND: Added deep rose-pink and bright crimson into the gradient mix */}
      ...
      >
        {Array.from({ length: rows * cols }).map((_, i) => (
          <div key={i} className="relative w-full h-full">
            ...
            />
            <div className="absolute inset-0 border-[1px] border-white/[0.04] pointer-events-none" />
          </div>
        ))}
      </div>
    </div>
